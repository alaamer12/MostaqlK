...
1. Run the listing script to check if the target process has an active tray item:
   ```powershell
   powershell -ExecutionPolicy Bypass -File .cursor/skills/tray-inspection/scripts/list-tray-items.ps1
   ```
...
1. **Direct PNG Extraction:**
   ...
   ```powershell
   powershell -ExecutionPolicy Bypass -File .cursor/skills/tray-inspection/scripts/extract-tray-icons.ps1 -OutDir "temp-tray-icons"
   ```
