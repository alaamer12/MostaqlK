...
Colors, theming, RTL, typography, and component base are specified separately in [`DESIGN.md`](./DESIGN.md) — this doc covers layout and interaction, that one covers the visual tokens.
...
Supports the full [dynamic query builder](./search-and-filtering.md#query-builder-ux) for filtering/sorting the feed, and the [search box](./search-and-filtering.md) for fuzzy title/description lookup.
...
- `unread only` is available as a standard filter chip in the [query builder](./search-and-filtering.md#query-builder-ux), since `is_read` is just another boolean column in the same filter system.
