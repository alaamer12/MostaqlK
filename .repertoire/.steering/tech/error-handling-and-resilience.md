...
- Logged, and the tray icon reflects an error state (see [ui-ux-design.md](./ui-ux-design.md#tray-icon) — MVP only needs a minimal error indicator, not the full polished state set).
...
- Parser code should be isolated to a single module/class per page type (listing parser, detail parser) so a markup change is a one-file fix — this was already a design goal in [overview.md](./overview.md#core-idea) and should be enforced as an actual module boundary, not just a convention.
