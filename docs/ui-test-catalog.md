...
| Refresh `↻` icon | `Projects_RefreshLabel` | Clickable, animated (feedback), dynamic-data | `ProjectFeedViewModel.RefreshCommand` → `RefreshAsync` → `LoadAsync`, updates `LastScanText` | `MainWindowPage.xaml:266-275` | `ProjectsPageTests.Click_Refresh_UpdatesLastScanText` / `Click_Refresh_Twice_NoDoubleFire` (counterpart) |
