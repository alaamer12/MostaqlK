...
| `CrashReporter` | static class (`Services/Diagnostics/CrashReporter.cs`) | Dedicated, thread-aware diagnostics logger recording managed thread IDs, thread pool flags, process uptime, working set memory, and complete inner exception hierarchies synchronously to `crash.log`. | Implemented |
