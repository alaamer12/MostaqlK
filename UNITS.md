...
|---|---|---|---|
| `CurrentPlatform` | static class (`Core/Platform/CurrentPlatform.cs`) | Single canonical place to read "what platform is this build" (`AppPlatform.Windows/Android/iOS/MacCatalyst`), resolved once via `#if` at compile time and exposed as `CurrentPlatform.Current`/`IsWindows`/`IsMobile`. Never re-detected at runtime — the running platform is fixed for the process's lifetime. Every capability-mapping site and future cross-platform audit should read the platform from here instead of ad hoc `#if WINDOWS` checks. | Implemented |
