...
| `AppPaths` / `IAppDirectoryProvider` | static class & interface (`Core/Platform/AppPaths.cs`) | Cross-platform directory provider standardizing application root (`AppDirectory`), data (`Data/`), diagnostic logs (`log/`), settings (`Settings/`), caches (`Cache/`), single-file bundle extraction cache (`bundle-cache/`), native crash minidumps (`log/crash.dmp`), attachments (`Cache/attachments`), database (`Data/mostaqlk.db`), and preferences (`Settings/preferences.dat`) across Windows (`%LocalAppData%\MostaqlK`), macOS (`~/Library/Application Support/MostaqlK`), Linux (`~/.local/share/mostaqlk`), Android, and iOS. Cleans up legacy/buggy folders (`User Name/`, `com.companyname.mostaqlk`). | Implemented |
...
| `CrashReporter` | static class (`Services/Diagnostics/CrashReporter.cs`) | Dedicated, thread-aware diagnostics and crash service recording managed and native Win32 SEH faults (`SetUnhandledExceptionFilter`, `MiniDumpWriteDump` minidump creation to `log/crash.dmp`, thread context, memory, and uptime) synchronously to `crash.log` and reporting post-mortem incidents with minidump telemetry to Sentry on subsequent launches. `RegisterGlobalHandlers` is the **single** registration site for the `AppDomain`/`TaskScheduler`/SEH hooks — it is idempotent (`Interlocked`) so calling it from both `Program.Main` and `MauiProgram` is fine, but no call site may add its own duplicate subscriptions; its managed handlers feed both `crash.log` and `InteractionLogger.Fault`. | Implemented |
| `BundleExtractionGuard` | internal static class (`Platforms/Windows/BundleExtractionGuard.cs`) | Pins the .NET single-file bundle extraction directory against deletion for the process lifetime. The apphost extracts to disk *before* any managed code runs, so a portable single-exe can never relocate its own extraction — `DOTNET_BUNDLE_EXTRACT_BASE_DIR` set inside the process is inert, and the User-scope value written by `Program.EnsurePersistentBundleExtraction` only helps *later* launches. Everything still lands in `%TEMP%`, which temp maintenance exists to clear. Memory-mapped images (the DLLs) are already immune because the OS refuses to delete a mapped file, but the loose assets this app reads back by absolute path at runtime — the `icon_*.png` behind `AppIcon`, the Tajawal `.ttf` faces, WinUI's `.pri`/`.mui` — are not. `PinDirectory` opens a `FileShare.Read` handle (deliberately omitting `FileShare.Delete`) on every file in the tree and holds it, so a cleanup tool's `DeleteFile` fails with a sharing violation. Best-effort, never throws, capped at 8192 handles. Called from `InitializeWindowsAppRuntime` immediately after `GetModuleFileName` resolves `Microsoft.WindowsAppRuntime.dll`, pinning that module's parent directory — the actual extraction tree, not `AppContext.BaseDirectory`. | Implemented |
...
| `AppCard` | `Border` | Project-feed card surface; carries `IsUnread`/`IsRead` bindable state for the unread accent-border treatment. Colors (surface background, accent/read border) are set in code from `UpdateThemeColors()` and re-applied on `Application.RequestedThemeChanged` — subscribed on `Loaded`, released on `Unloaded` per the Event lifetime rule, since a card is created per feed item — so both the read and unread states have real light/dark parity instead of one hardcoded color pair. | Implemented |
...
| Unit | Type | Purpose | Status |
|---|---|---|---|
| `TrayIconService` | class | Windows system-tray icon: `TrayIconState` mirrored live from `IPollService.StatusChanged`/`DiscoveryQueue.Count` + right-click menu wired to real commands (Open, Pause/Resume, Check now, Recent notifications, Settings, Quit). Native icon hosting via `Platforms/Windows/TrayIconNativeHost.cs` (`Shell_NotifyIcon`). Dynamic rounded-centroid brand icons with status badges (Orange = Idle, Blue = Polling/Pulling, Green = Processing/Backlog Draining, Red = Error) bundled as multi-size `.ico` files and embedded resources. Resolved in DI via `PlatformCapability<TrayIconService>.WindowsOnly(...)` (not unconditional construction) so the "null on mobile" answer is explicit and typed through the shared platform-capability utility; call sites already null-check/`?.` and no-op when absent. Windows behavior is unchanged. | Implemented |
...
`Application.Current` is an **app-lifetime publisher**. Subscribing to it (`RequestedThemeChanged`,
and anything else) from a **constructor** roots that instance permanently — and through
`view.Handler → MauiContext → PlatformView`, its live native peer too. For a unit instantiated per
feed item (`AppCard`, `NewRibbonBadge`) or per realized page (`AppSidebar`), that grows for the
whole session, which is how a long-running portable exe accumulates thousands of dead views.

Bind such subscriptions to the view's own lifetime instead:

- `Loaded` → `+=`, `Unloaded` → `-=`; or, when the unit already has handler-scoped wiring
  (`PipelineRadar`), do both in `OnHandlerChanged` so a recycled handler re-themes correctly.
- Use a **named method or a local**, never an inline lambda — `-=` cannot match a lambda.
- **Minus before plus.** `Loaded` can fire more than once for one instance and `+=` is not idempotent.
- Re-apply the theme on load: a flip that happened while the view was off screen must still land.
